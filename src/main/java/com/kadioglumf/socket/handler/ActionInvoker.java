...
public class ActionInvoker {
  ...
  private void bootstrap() {
    ...
    for (Method method : methods) {
      ...
      // log.debug("Mapped action `{}` in channel handler `{}#{}`", action, handlerClass.getName(),
      // method);
    }
  }
  ...
  public void invokeAction(
    ...
    try {
      ...
      if (e instanceof InvocationTargetException
          && ((InvocationTargetException) e).getTargetException() instanceof WebSocketException) {
        WebSocketException exception =
            (WebSocketException) ((InvocationTargetException) e).getTargetException();
